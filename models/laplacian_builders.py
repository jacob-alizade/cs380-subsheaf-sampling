...
class NoisyNormConnectionLaplacianBuilder(NormConnectionLaplacianBuilder):
    """Learns a a Sheaf Laplacian with diagonal restriction maps and gauge noise"""

    def __init__(self, *args, delta=1e-4, **kwargs):
    ...
    def forward(self, map_params, edge_weights=None):
        ...
        #apply noise
        G = torch.randn_like(maps)
        A = 0.5 * (G - G.transpose(-1, -2))        # (E, d, d), in so(d)
        expA = torch.linalg.matrix_exp(self.delta * A)  # (E, d, d) noise rotations

        maps = torch.bmm(maps, expA)
...
class SubsheafLaplacianBuilder(LaplacianBuilder):
    def __init__(self, size, edge_index, k, 
                 normalised=True, deg_normalised=False,
                 add_hp=False, add_lp=False, augmented=True):
        # Initialize exactly as the base, but with d=k
        super().__init__(
            size=size,
        ...
        _, self.tril_indices = lap.compute_learnable_laplacian_indices(
            size, self.vertex_tril_idx, k, self.final_d
        )
        self.diag_indices, _ = lap.compute_learnable_diag_laplacian_indices(
            size, self.vertex_tril_idx, k, self.final_d
        )

    def forward(self, maps_k, edge_weights=None):
        """
        maps_k: Tensor (E, k, k) of already‐orthogonal transport maps.
        edge_weights: optional (E,1)
        """
        row, _ = self.edge_index
        ...
        if edge_weights is None:
            ...
            diag_maps = scatter_add(edge_weights**2, row, dim=0, dim_size=self.size)
            maps_k   = maps_k * edge_weights.view(-1,1,1)

        # Off‐diagonal blocks
        left_idx, right_idx = self.left_right_idx
        Lm = maps_k[left_idx]    # (E, k, k)
        Rm = maps_k[right_idx]   # (E, k, k)
        tril_maps = -torch.bmm(Lm.transpose(-1,-2), Rm)  # (E, k, k)
        #print(f"{tril_maps.shape=}, {left_idx.shape=}, {right_idx.shape=}, {diag_maps.shape=}")
        # Normalize
        tril_row, tril_col = self.vertex_tril_idx
        diag_maps, tril_maps = self.scalar_normalise(
            diag_maps, tril_maps, tril_row, tril_col
        )
        tril_flat = tril_maps.view(-1)
        diag_flat = diag_maps.expand(-1, self.d).reshape(-1)

        # Append HP/LP if needed
        (diag_indices, diag_flat), (tri_indices, tril_flat) = \
            self.append_fixed_maps(len(Lm),
                                   self.diag_indices, diag_flat,
                                   self.tril_indices, tril_flat)

        # Symmetrize & merge into COO
        triu_idx = torch.stack([tri_indices[1], tri_indices[0]], dim=0)
        #print(f"{tri_indices.shape=}, {tril_flat.shape=}, {triu_idx.shape=}, {tril_flat.shape=}")
        non_diag_idx, non_diag_val = lap.mergesp(
            tri_indices, tril_flat, triu_idx, tril_flat
        )
        edge_idx, weights = lap.mergesp(
            non_diag_idx, non_diag_val, diag_indices, diag_flat
        )

        return (edge_idx, weights), tril_flat.detach().clone()

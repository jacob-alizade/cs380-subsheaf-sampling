...
from models.disc_models import DiscreteDiagSheafDiffusion, DiscreteBundleSheafDiffusion, DiscreteGeneralSheafDiffusion, DiscreteSampleBundleSheafDiffusion
...
def test_sample_bundle_diffusion_backprop(d, orth, k):
    ...
    with profile(
        activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],   # add ProfilerActivity.CUDA if you’re on CUDA
        record_shapes=True,
        with_stack=True
    ) as prof:
        with record_function("model_forward"):
            ...
            #     print(f"{name:30s} grad: {param.grad is not None}")
            # Check that gradients are back-propagated through the Laplacian in particular
            for param in model.parameters():
                assert param.grad is not None
...
@pytest.mark.parametrize("d, orth", [(2, "euler"), (3, "euler"), (5, "matrix_exp"), (6, "cayley"), (7, "householder")])
def test_permutation_equivariance_of_sample_bundle_sheaf_diffusion_on_texas(d, orth, k = None):
    if not k or k < 0:
        k = d
    # Fix the random seed
    torch.random.manual_seed(0)
    ...
    perms = 3
    permutation_matrices = generate_permutation_matrices(size=data.x.size(0), amount=perms)
    ...
    for i in range(perms):
        with torch.no_grad():
            model = DiscreteSampleBundleSheafDiffusion(data.edge_index, args, k, 1)

            if P is None:
                I = torch.eye(d)
                P = I.unsqueeze(0).repeat(data.x.size(0), 1, 1)

            model.eval()
            out = model(data.x, P_override = P)
            out = torch.FloatTensor(permutation_matrices[i] @ out.numpy())

            perm_data = permute_graph(data.clone(), permutation_matrices[i])
            model.update_edge_index(perm_data.edge_index)
            perm_out = model(perm_data.x)
...
@pytest.mark.parametrize("d, orth", [(2, "householder")])
def test_sample_bundle_equivalence(d, orth):
    ...
    base_model = DiscreteBundleSheafDiffusion(data.edge_index, args)
    base_model.eval()

    sample_model = DiscreteSampleBundleSheafDiffusion(data.edge_index, args, d, 1)
    sample_model.load_state_dict(base_model.state_dict(), strict=False)
    sample_model.eval()

    I = torch.eye(d)
    P = I.unsqueeze(0).repeat(data.x.size(0), 1, 1)

    base_out = base_model(data.x)
    sample_out = sample_model(data.x, P_override = P)

    print(f"{base_out=}\n{sample_out=}")

    assert torch.allclose(base_out, sample_out, atol=1e-3)


if __name__ == "__main__":
    test_sample_bundle_diffusion_backprop(7, "householder", 2)